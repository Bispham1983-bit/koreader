...
local B_SETT = {	--BANNER SETTINGS
					...
					min_width_hl_off = 40,	-- minimum width when highlight off, min: 20
					min_width_hl_on = 45,	-- minimum width when highlight on, min: 20
					max_width_hl_off = 50,	-- width when highlight off, min: 20
...
function UIManager:show(widget, ...)
	...
	if not highlightEnabled then
		min_wid = B_SETT.min_width_hl_off and
					B_SETT.min_width_hl_off >= 20 and
					B_SETT.min_width_hl_off <= 100 and
					(B_SETT.min_width_hl_off/100 * screen_w) or
					screen_w * 0.35
		min_wid = min_wid - overflow_w
		...
		min_wid = B_SETT.min_width_hl_on and
					B_SETT.min_width_hl_on >= 20 and
					B_SETT.min_width_hl_on <= 100 and
					(B_SETT.min_width_hl_on/100 * screen_w) or
					screen_w * 0.45
		min_wid = min_wid - overflow_w_hl
	...
	content_widget = VerticalGroup:new{
		align = "left",
		HorizontalSpan:new{width = min_wid},
		title_widget,
		stats_widget,			
	}
